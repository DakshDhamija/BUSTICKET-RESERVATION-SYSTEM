        ...
        for vehicle in self.vehicles:
            if vehicle.vehicle_number == vehicle_number:
                for trip in vehicle.trips:
                    if (trip.pick_up_location == pick_up_location and
                        trip.drop_location == drop_location and
                        trip.departure_time == departure_time and
                        trip.passenger_details and
                        trip.passenger_details['name'] == passenger_name and
                        trip.passenger_details['age'] == passenger_age and
                        trip.passenger_details['gender'] == passenger_gender):
                        booked_seats = trip.get_booked_seats()
                        if booked_seats > 0:
                            trip.set_booked_seats(booked_seats - 1)
                            vehicle.set_seating_capacity(vehicle.get_seating_capacity())
                            trip.passenger_details = None
                            ...
                            with open("ticket_log.txt", "w") as file:
                                for line in lines:
                                    ...
                                    if len(parts) >= 7 and parts[:7] != [pick_up_location, drop_location, vehicle_number, str(departure_time), passenger_name, str(passenger_age), passenger_gender]:
                                        file.write(line)
                            return trip
        return None
